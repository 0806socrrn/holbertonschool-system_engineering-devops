# 0x0B. SSH
## Details
 By: Sylvain Kalache Weight: 1Ongoing second chance project - startedAug 22, 2022 12:00 AM, must end byAug 27, 2022 12:00 AM An auto review will be launched at the deadline#### In a nutshell…
...
## Tasks
### 0. Use a private key
          mandatory         Progress vs Score           Score: 0.00% (Checks completed: 0.00%)         Task Body Write a Bash script that uses   ` ssh `   to connect to your server using the private key   ` ~/.ssh/school `   with the user   ` ubuntu `  .
Requirements:
...
### 1. Create an SSH key pair
          mandatory         Progress vs Score           Score: 0.00% (Checks completed: 0.00%)         Task Body Write a Bash script that creates an RSA key pair.
Requirements:
...
### 2. Client configuration file
          mandatory         Progress vs Score           Score: 0.00% (Checks completed: 0.00%)         Task Body Your machine has an SSH configuration file for the local SSH client, let’s configure it to our needs so that you can connect to a server without typing a password.Share your SSH client configuration in your answer file.
Requirements:
...
* File:  ` 2-ssh_config ` 
 Self-paced manual review  Panel footer - Controls 
### 3. Let me in!
          mandatory         Progress vs Score           Score: 0.00% (Checks completed: 0.00%)         Task Body Now that you have successfully connected to your server, we would also like to join the party.
...
### 4. Client configuration file (w/ Puppet)
          #advanced         Progress vs Score           Score: 0.00% (Checks completed: 0.00%)         Task Body Let’s practice using Puppet to make changes to our configuration file. Just as in the previous configuration file task, we’d like you to set up your client SSH configuration file so that you can connect to a server without typing a password.
Requirements:
* Your SSH client configuration must be configured to use the private key  ` ~/.ssh/school ` 
* Your SSH client configuration must be configured to refuse to authenticate using a password
Example:
...
#### Port mapping
SSH49162HTTP49161HTTPS49160300049159MySQL49158400049157500049156500149155800049154808049153### Ubuntu 20.04 - Fabric 3 & Puppet
